...
public class ProfitAccount {
    public static final String packagingParam(Context context, String... kvs) {
        ...
        try {
            ...
            paramJsonObject.put("trad_no", Escape.escape(kvs[2]));
            paramJsonObject.put("pay_way", Escape.escape(kvs[3]));
            paramJsonObject.put("pay_account", Escape.escape(kvs[4]));
            paramJsonObject.put("total_fee", Escape.escape(kvs[5]));
            //这里需要提醒一下果冻，将ID号连接成("1,2,3,4")形式的字符串
            paramJsonObject.put("consume_id", Escape.escape(kvs[6]));
